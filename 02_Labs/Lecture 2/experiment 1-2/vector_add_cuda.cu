...
#define CUDA_CHECK(call)                                                     \
    do {                                                                     \
        ...
        // if the error is not cudaSuccess, print an error message and exit the program
        // cudaSuccess is a constant defined in cuda_runtime.h, it determines if the CUDA operation was successful or not
        // if the error is not cudaSuccess, print an error message and exit the program
        // cudaGetErrorString is a function that returns a string describing the error
        
        // std::cerr is a stream object that writes to the standard error stream, 
        
        // __FILE__ is the name of the file, __LINE__ is the line number
        // std::exit is a function that exits the program
